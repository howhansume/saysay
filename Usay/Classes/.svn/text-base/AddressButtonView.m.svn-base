...
@synthesize msgButton, smsButton, callButton, recid;
...
- (id)initFrame:(CGRect)frame falg:(int)number {
	...
	if(self != nil) {
		...
		//	DebugLog(@"fdsaf = %@", [parentView.rootController.viewControllers objectAtIndex:0]);
		
		UINavigationController *tmp = [parentView.rootController.viewControllers objectAtIndex:0];
		
		//	DebugLog(@"abba= %@", [tmp.viewControllers objectAtIndex:0]) ;
...
-(void)layoutSubviews 
{
	...
	if(self.isBuddy)
	{
		NSLog(@"layout subview YES");
		self.msgButton.enabled = YES;
	}
	else
	{
		NSLog(@"layout subview NO");
		self.msgButton.enabled = NO;
	}
...
- (void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if ([allTouches count] == 1)
	{
		...
		if ([touch tapCount] == 1)
		{
			...
			//	DebugLog(@"touch = %f %f", touchBeganX, touchBeganY);
