...
//	DebugLog(@"addGroup >> %@",[json stringWithObject:bodyObject error:nil]);
//	[json release];
...
-(BOOL)requestUpdateGroup:(GroupInfo*)GroupData {
	...
	SBJSON *json = [[SBJSON alloc] init];
	[json setHumanReadable:YES];
	DebugLog(@"updateGroup >> %@",[json stringWithObject:bodyObject error:nil]);
	[json release];
...
//			}
			...
			NSString *userUpdate = [NSString stringWithFormat:@"update _TUserInfo set OEMMODIFYDATE='%@' where gid='%@'", lastSyncDate, groupData.ID];
			//release 2010.12.28
			[lastSyncDate release];
			
			DebugLog(@"update user Quert = %@", userUpdate);
			[UserInfo findWithSql:userUpdate];
		}
	}
