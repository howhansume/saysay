...
// Connecting (timeout 처리)
- (void)startConnectTimeout:(NSTimeInterval)timeout;
- (void)endConnectTimeout;

// Socket Implementation
// Server Accept 용 소켓 생성
...
{
//	NSLog(@"ASyncSocket connectToAddress start");
	if (theDelegate == NULL)
	{
		NSString *message = @"Attempting to connect without a delegate. Set a delegate first.";
...
//	NSLog(@"ASyncSocket connectToAddress end");
	return pass;
}
...
- (CFSocketRef)createAcceptSocketForAddress:(NSData *)addr error:(NSError **)errPtr
{
	...
	// 110125 analyze
	if(theCreateSocket != NULL)
	{
		CFSocketInvalidate(theCreateSocket);
		CFRelease(theCreateSocket);
		theCreateSocket = NULL;
	}
	
	// 110125 analyze
//	CFSocketRef socket = CFSocketCreate(kCFAllocatorDefault,
	theCreateSocket = CFSocketCreate(kCFAllocatorDefault,
										addressFamily,
										SOCK_STREAM,
										0,
	...
	//[socket release];
	//2010.01.11 autorelease 추가
	return theCreateSocket;
}

- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
//	NSLog(@"ASyncSocket createSocketForAddress 소켓 생성 start");
...
//	NSLog(@"ASyncSocket createSocketForAddress 소켓 생성 end");
	return YES;
}
...
- (BOOL)attachSocketsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{	
//	NSLog(@"ASyncSocket attachSocketsToRunLoop start");
	// Get the CFRunLoop to which the socket should be attached.
	theRunLoop = (runLoop == nil) ? CFRunLoopGetCurrent() : [runLoop getCFRunLoop];

	if(theSocket) {
		theSource  = CFSocketCreateRunLoopSource (NULL, theSocket, 0);
...
//	NSLog(@"ASyncSocket attachSocketsToRunLoop end");
	return YES;
}
...
- (BOOL)connectSocketToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
//	NSLog(@"ASyncSocket connectSocketToAddress start");
	...
	if(theSocket) {
		...
		// -1은 timeout 이며 초단위 지정
		// 음수값이면 CFSocketConnectToAddress 는 즉각 반환되며, 백그라운드에서 접속을 시도
		// 접속이 이루어지면 Socket 생성때 사용했던 Callback 함수가 호출
...
//	NSLog(@"ASyncSocket connectSocketToAddress end");
	return YES;
}
...
{
//	NSLog(@"ASyncSocket doSocketOpen");
...
- (BOOL)createStreamsFromNative:(CFSocketNativeHandle)native error:(NSError **)errPtr
{
//	NSLog(@"ASyncSocket createStreamsFromNative read/write stream 생성");
...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
//	NSLog(@"ASyncSocket attachStreamsToRunLoop");
...
- (void)doBytesAvailable
{
	...
	if(theCurrentRead != nil && theReadStream != NULL) {
		...
		while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
		{
			...
			theCurrentRead->readAllAvailableData = YES;
...
//	NSLog (@"doCFSocketCallback");
	...
	switch (type)
	{
		case kCFSocketConnectCallBack:
			...
			if(pData) {
//				NSLog(@"ASyncSocket doCFSocketCallback kCFSocketConnectCallBack Error");
				[self doSocketOpen:sock withCFSocketError:kCFSocketError];
			} else {
//				NSLog(@"ASyncSocket doCFSocketCallback kCFSocketConnectCallBack Success");
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	NSLog(@"여기..");
	//2011.01.20 스트림이 널인 경우에 죽는 현상이 있어 예외 처리함..
	if(theReadStream == NULL || theReadStream == nil)
	{
		NSLog(@"스트림 에러 ");
		return;
	}
	...
	if(theReadStream == nil || theReadStream == NULL)
		return;
	...
	switch (type)
	{
		case kCFStreamEventOpenCompleted:	// The open has completed successfully.
//			NSLog (@"doCFReadStreamCallback kCFStreamEventOpenCompleted");
			theFlags |= kDidCompleteOpenForRead;
			[self doStreamOpen];
			break;
		case kCFStreamEventHasBytesAvailable:	// The stream has bytes to be read. 
			if(theFlags & kStartingReadTLS) {
//				NSLog (@"doCFReadStreamCallback kCFStreamEventOpenCompleted kStartingTLS");
...
//				NSLog (@"doCFReadStreamCallback kCFStreamEventOpenCompleted");
			...
			break;
		case kCFStreamEventErrorOccurred:		// 오류 처리
			NSLog (@"doCFReadStreamCallback kCFStreamEventErrorOccurred");
			err = CFReadStreamGetError (theReadStream);
			//이 부분에서 죽는다는데 예외 처리를 한번 해보자 2011.01.24 걍 위에서 예외 처리함..
				[self closeWithError: [self errorFromCFStreamError:err]];
			break;
		case kCFStreamEventEndEncountered:		// 스트림 종료시
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		case kCFStreamEventOpenCompleted:	// The open has completed successfully.
		{
//			NSLog (@"doCFWriteStreamCallback kCFStreamEventOpenCompleted");
			theFlags |= kDidCompleteOpenForWrite;
			[self doStreamOpen];
			break;
		}
		case kCFStreamEventCanAcceptBytes:	// The stream can accept bytes for writing.
		{
			if(theFlags & kStartingWriteTLS) {
//				NSLog (@"doCFWriteStreamCallback kCFStreamEventCanAcceptBytes kStartingTLS");
				[self onTLSStarted:YES];
			} else {
//				NSLog (@"doCFWriteStreamCallback kCFStreamEventCanAcceptBytes");
				[self doSendBytes];
			}
			break;
		}
		case kCFStreamEventErrorOccurred:	// An error has occurred on the stream.
		{
//			NSLog (@"doCFWriteStreamCallback kCFStreamEventErrorOccurred");
			err = CFWriteStreamGetError (theWriteStream);
			[self closeWithError: [self errorFromCFStreamError:err]];
			break;
		}
		case kCFStreamEventEndEncountered:	// The end of the stream has been reached.
		{
//			NSLog (@"doCFWriteStreamCallback kCFStreamEventEndEncountered");
			err = CFWriteStreamGetError (theWriteStream);
			[self closeWithError: [self errorFromCFStreamError:err]];
			break;
		}
...
//		NSLog(@"MyCFSocketCallback kCFSocketConnectCallBack");
