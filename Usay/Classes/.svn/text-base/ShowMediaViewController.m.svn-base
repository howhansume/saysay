...
-(void)savePictureToLibraryClicked
{
	...
	//중복으로 누르는 현상 하지 못하게 막는다.. 2011.02.09 kjh
	if(saveflag == YES)
		return;
...
//	moviePlayer.movieControlMode = MPMovieControlModeHidden;	// 동영상 컨트롤러 히든. 기본값은 show.
	...
	/*여기에서 릴리즈 하면안된다 왜나하면 노트 끝나는 시점에 접근이 안되기 때문이다.*/
	//release 2010.12.28
//	[moviePlayer release];
...
{	
	...
	//	[moviePlayer release];	// sochae 2010.10.18 - (주석 해지) 2011.03.03 release 할수가 없음
	}
	//메모리 해제..
	[moviePlayer release];
