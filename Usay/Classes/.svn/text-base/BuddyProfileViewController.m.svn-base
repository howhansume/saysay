...
//	isMemoryWarning = NO;
	...
	DebugLog(@"프로필 유저 정보 : %@", currentUser);
...
-(void)imgClick
{
	...
	[mainImageView setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:imgButton.imageView.image];
...
-(void) createCustomHeaderView {
	...
	DebugLog(@"createCustomHeaderView");
	...
	//로컬디비에 작은 이미지가 존재하면 보여주고 안그럼 디폴트 이미지로 나간다.. 이부분은 그냥 서버로 수정하자..
...
//	assert(nameLabel != nil);
	...
	nameLabel.text = profileUserData.PROFILENICKNAME;
...
-(void)addCustomFooterView {
	...
	//kjh 2010.11.04 서버에서 내려주는 데이터3 개가 없으면 비활성
	DebugLog(@"mobile = %@", self.userProfile.MOBILEPHONENUMBER);
	DebugLog(@"iphone = %@", self.userProfile.IPHONENUMBER);
	DebugLog(@"main = %@", self.userProfile.MAINPHONENUMBER);
	if([self.userProfile.MOBILEPHONENUMBER length]<=0 && [self.userProfile.IPHONENUMBER length]<=0 && [self.userProfile.HOMEPHONENUMBER length]<=0)
	{
...
-(void)profileSMSSendClicked
{
	...
	if (self.userProfile.MOBILEPHONENUMBER != nil && [self.userProfile.MOBILEPHONENUMBER length] > 0) {
		phoneNumber = [NSString stringWithFormat:@"%@", self.userProfile.MOBILEPHONENUMBER];
	} else if (self.userProfile.IPHONENUMBER && [self.userProfile.IPHONENUMBER length] > 0) {
		phoneNumber = [NSString stringWithFormat:@"%@",self.userProfile.IPHONENUMBER];
	} else if (self.userProfile.HOMEPHONENUMBER && [self.userProfile.HOMEPHONENUMBER length] > 0) {
		phoneNumber = [NSString stringWithFormat:@"%@",self.userProfile.HOMEPHONENUMBER];
	}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
	...
	DebugLog(@"sectioncount %i",[tableItem count]);
...
#if TARGET_IPHONE_SIMULATOR	
	DebugLog(@"section%i rowcount %i",section,[[tableItem objectForKey:key] count]);
#endif
	DebugLog(@"section%i rowcount %i",section,[[tableItem objectForKey:key] count]);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(section == 0){
		...
		NSLog(@"group cnt = %d", [groupInfo count]);
...
{
	DebugLog(@"formatted %@		nickname", self.profileUserData.FORMATTED, self.profileUserData.PROFILENICKNAME);
	self.tableArray = [GroupInfo findByColumn:@"ID" value:self.profileUserData.GID]; //group
...
*/	
	//2010.10.26 프로파일-포맷티드로 변경
	// [modify] sochae - 프로필 화면은 프로필 닉네임으로만 표시.
	DebugLog(@"프로필 닉네임 비교 (%@ = db %@)", self.userProfile.NICKNAME, self.profileUserData.PROFILENICKNAME);
	if(self.userProfile.NICKNAME != nil && [self.userProfile.NICKNAME length] > 0){
		NSLog(@"nickname");
		UILabel *titleLabel = (UILabel *)[self.view viewWithTag:kTagTitleLabel];
		titleLabel.text = self.userProfile.NICKNAME;
	}/*
	else if(self.profileUserData.FORMATTED != nil && [self.profileUserData.FORMATTED length] > 0){
		
		NSLog(@"프로파일 닉네임???");
		
		UILabel *titleLabel = (UILabel *)[self.view viewWithTag:kTagTitleLabel];
		titleLabel.text = self.profileUserData.FORMATTED;
		profileUserData.PROFILENICKNAME = [NSString stringWithFormat:@"%@",self.profileUserData.FORMATTED];
	}
	  */
	else {
		UILabel *titleLabel = (UILabel *)[self.view viewWithTag:kTagTitleLabel];
	...
	*/
	...
	DebugLog(@"subtitle = %@", subTitleLabel.text);
	...
	UIButton *smsButton = (UIButton*)[self.view viewWithTag:9999];
	[smsButton addTarget:self action:@selector(profileSMSSendClicked) forControlEvents:UIControlEventTouchUpInside];
	
	if([self.userProfile.MOBILEPHONENUMBER length]<=0 && [self.userProfile.IPHONENUMBER length]<=0)
	{
		[smsButton setEnabled:NO];
	}
	else {
...
//			assert(commStatus != nil);
		...
		// 200 이외의 에러
		UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"실패" message:@"친구 정보를 찾을 수 없습니다." delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
		[alertView show];
		[alertView release];
		return;
