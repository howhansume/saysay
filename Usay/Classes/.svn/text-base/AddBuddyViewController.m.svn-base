...
-(void)updateDataValue:(NSString*)value forkey:(NSString*)key{
	...
	if([key isEqualToString:@"IPHONENUMBER"])
	{
		[[rowKeys objectAtIndex:2] insertObject:@"IPHONENUMBER" atIndex:1];
		[[rowLabels objectAtIndex:2] insertObject:@"핸드폰2" atIndex:1];
		[[rowControllers objectAtIndex:2] insertObject:@"ManagedObjectStringEditor" atIndex:1];
	}
	else if([key isEqualToString:@"ORGEMAILADDRESS"])
	{
		...
		if([rowKeys count] == 9)
		{
			[[rowKeys objectAtIndex:2] insertObject:@"ORGEMAILADDRESS" atIndex:5];
			[[rowLabels objectAtIndex:2] insertObject:@"이메일2" atIndex:5];
			[[rowControllers objectAtIndex:2] insertObject:@"ManagedObjectStringEditor" atIndex:5];
			
		}
		else {
			[[rowKeys objectAtIndex:2] insertObject:@"ORGEMAILADDRESS" atIndex:4];
			[[rowLabels objectAtIndex:2] insertObject:@"이메일2" atIndex:4];
			[[rowControllers objectAtIndex:2] insertObject:@"ManagedObjectStringEditor" atIndex:4];
		}
	...
	NSLog(@"key %@ value %@ %d", key, value, indexPath.row);
...
*/	
	...
	for(NSString* addkey in keyArray) {
		...
		if([addkey isEqualToString:@"HOMEEMAILADDRESS"])
			userdata.HOMEEMAILADDRESS = [self.addBuddyDic objectForKey:addkey];
		...
		//기타 정보들 추가 2011.01.12
		
		
		if([addkey isEqualToString:@"MOBILEPHONENUMBER"])
			userdata.MOBILEPHONENUMBER  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"HOMEPHONENUMBER"])
			userdata.HOMEPHONENUMBER  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"IPHONENUMBER"])
			userdata.IPHONENUMBER  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"ORGPHONENUMBER"])
			userdata.ORGPHONENUMBER  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"HOMEEMAILADDRESS"])
			userdata.HOMEEMAILADDRESS  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"ORGEMAILADDRESS"])
			userdata.ORGEMAILADDRESS  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"ORGNAME"])
			userdata.ORGNAME  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"NICKNAME"])
			userdata.NICKNAME  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"URL1"])
			userdata.URL1  = [self.addBuddyDic objectForKey:addkey];
		if([addkey isEqualToString:@"NOTE"])
			userdata.NOTE  = [self.addBuddyDic objectForKey:addkey];
	...
	else if((userdata.MOBILEPHONENUMBER == nil || userdata.MOBILEPHONENUMBER == NULL) &&
			(userdata.IPHONENUMBER == nil || userdata.IPHONENUMBER == NULL) &&
			(userdata.HOMEPHONENUMBER == nil || userdata.HOMEPHONENUMBER == NULL) &&
			(userdata.ORGPHONENUMBER == nil || userdata.ORGPHONENUMBER == NULL) &&
			(userdata.HOMEEMAILADDRESS == nil || userdata.HOMEEMAILADDRESS == NULL) &&
			(userdata.ORGEMAILADDRESS == nil || userdata.ORGEMAILADDRESS == NULL) &&
			(userdata.ORGNAME == nil || userdata.ORGNAME == NULL) &&
			(userdata.NICKNAME == nil || userdata.NICKNAME == NULL) &&
			(userdata.URL1 == nil || userdata.URL1 == NULL) &&
			(userdata.NOTE== nil || userdata.NOTE == NULL)
			)
	{
		UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"입력 정보 확인" 
															message:@"전화번호 또는 이메일를 입력하셔야 추가 하실수 있습니다." 
														   delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil];
		[alertView show];
		[alertView release];
		return;
	...
	//		NSLog(@"pro %d", [[[self propertyValues] objectAtIndex:10] length]);
		...
		*/
		...
		DebugLog(@"pre fdsafdsfsadaf %@", strTemp);
		...
		NSLog(@"userData %@", userdata.HOMEPHONENUMBER);
...
//	[HeaderImageView release];
	...
	rowLabels = [[NSArray alloc] initWithObjects:
                 ...
                 // Section 3
                 [NSMutableArray arrayWithObjects:@"핸드폰", @"집전화", @"직장전화",@"이메일",@"소속", @"별명", @"홈페이지", @"메모", nil],
				// Sentinel
                 nil];
    ...
    rowKeys = [[NSArray alloc] initWithObjects:
               ...
               [NSArray arrayWithObjects:@"SELGROUP", nil],
               // Section 2
               [NSMutableArray arrayWithObjects:@"MOBILEPHONENUMBER",@"HOMEPHONENUMBER", @"ORGPHONENUMBER",@"HOMEEMAILADDRESS", @"ORGNAME", @"NICKNAME",
				@"URL1", @"NOTE", nil],
               // Sentinel
               nil];
	...
	rowControllers = [[NSArray alloc] initWithObjects:
					  ...
					  //section 3
                      [NSMutableArray arrayWithObjects:@"ManagedObjectStringEditor", @"ManagedObjectStringEditor", @"ManagedObjectStringEditor",@"ManagedObjectStringEditor",
					   @"ManagedObjectStringEditor",@"ManagedObjectStringEditor",@"ManagedObjectStringEditor",@"ManagedObjectStringEditor",nil],
					 // Sentinel
                      nil];
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView 
	...
	if([rowLabel isEqualToString:@"핸드폰"] ){
		if([self.addBuddyDic valueForKey:@"MOBILEPHONENUMBER"] != nil && [[self.addBuddyDic valueForKey:@"MOBILEPHONENUMBER"] length] > 0 &&
		  [self.addBuddyDic valueForKey:@"IPHONENUMBER"]  != nil && [[self.addBuddyDic valueForKey:@"IPHONENUMBER"] length] > 0)
			return UITableViewCellEditingStyleNone;
		else
			return UITableViewCellEditingStyleInsert;
	}
	if([rowLabel isEqualToString:@"이메일"] ){
		if([self.addBuddyDic valueForKey:@"HOMEEMAILADDRESS"] != nil && [[self.addBuddyDic valueForKey:@"HOMEEMAILADDRESS"] length] > 0 &&
		   [self.addBuddyDic valueForKey:@"ORGEMAILADDRESS"]  != nil && [[self.addBuddyDic valueForKey:@"ORGEMAILADDRESS"] length] > 0)
			return UITableViewCellEditingStyleNone;
		else
			return UITableViewCellEditingStyleInsert;
	}
	if([rowLabel isEqualToString:@"핸드폰2"] ){
		...
		if([[self.addBuddyDic valueForKey:@"MOBILEPHONENUMBER"] length] <= 0)
			return UITableViewCellEditingStyleInsert;
	}
	if([rowLabel isEqualToString:@"이메일2"] ){
		if([self.addBuddyDic valueForKey:@"HOMEEMAILADDRESS"] != nil && [[self.addBuddyDic valueForKey:@"HOMEEMAILADDRESS"] length] > 0)
			return UITableViewCellEditingStyleDelete;
		if([[self.addBuddyDic valueForKey:@"HOMEEMAILADDRESS"] length] <= 0)
			return UITableViewCellEditingStyleInsert;
	}
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (editingStyle == UITableViewCellEditingStyleInsert){
		...
		//핸드폰이나 이메일이 텅 비어있을 경우..
		if([[self.addBuddyDic valueForKey:rowKey] length] == 0){
		...
		[self.addBuddyDic setValue:@"" forKey:rowKey];
		
		
		
		
		[[rowKeys objectAtIndex:2] removeObjectAtIndex:indexPath.row];
		[[rowLabels objectAtIndex:2] removeObjectAtIndex:indexPath.row];
		[[rowControllers objectAtIndex:2] removeObjectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	//		cell.detailTextLabel.font = [UIFont boldSystemFontOfSize:14.0];
			cell.detailTextLabel.font = [UIFont boldSystemFontOfSize:0.0];
		...
		cell.textLabel.adjustsFontSizeToFitWidth=YES; //이거 안해주면 글자 ... 으로 된다 4글자 부터..
  ...
  if([rowKey isEqualToString:@"FORMATTED"]) {
	...
	NSLog(@"indexpath row %d title %@", indexPath.row, cell.textLabel.text);
...
-(BOOL)requestAddContact:(UserInfo*)addUserInfo{
	...
	if(addUserInfo.MOBILEPHONENUMBER != nil && addUserInfo.MOBILEPHONENUMBER != NULL)
	{
		[bodyObject setObject:addUserInfo.MOBILEPHONENUMBER forKey:@"mobilePhoneNumber"];
	}
	if(addUserInfo.IPHONENUMBER != nil && addUserInfo.IPHONENUMBER != NULL)
	{
		NSLog(@"핸드폰2 번호 %@", addUserInfo.IPHONENUMBER);
		[bodyObject setObject:addUserInfo.IPHONENUMBER forKey:@"iphoneNumber"];
	}
	if(addUserInfo.HOMEEMAILADDRESS != nil && addUserInfo.HOMEEMAILADDRESS != NULL)
	{
		[bodyObject setObject:addUserInfo.HOMEEMAILADDRESS forKey:@"homeEmailAddress"];
	}
	if(addUserInfo.ORGEMAILADDRESS != nil && addUserInfo.ORGEMAILADDRESS != NULL)
	{
		[bodyObject setObject:addUserInfo.ORGEMAILADDRESS forKey:@"orgEmailAddress"];
	}
	if(addUserInfo.HOMEPHONENUMBER != nil && addUserInfo.HOMEPHONENUMBER != NULL)
	{
		[bodyObject setObject:addUserInfo.HOMEPHONENUMBER forKey:@"homePhoneNumber"];
	}
	if(addUserInfo.ORGPHONENUMBER != nil && addUserInfo.ORGPHONENUMBER != NULL)
	{
		[bodyObject setObject:addUserInfo.ORGPHONENUMBER forKey:@"orgPhoneNumber"];
	}
	if(addUserInfo.ORGNAME != nil && addUserInfo.ORGNAME != NULL)
	{
		[bodyObject setObject:addUserInfo.ORGNAME forKey:@"orgName"];
	}
	if(addUserInfo.NICKNAME != nil && addUserInfo.NICKNAME != NULL)
	{
		[bodyObject setObject:addUserInfo.NICKNAME forKey:@"nickName"];
	}
	if(addUserInfo.URL1 != nil && addUserInfo.URL1 != NULL)
	{
		[bodyObject setObject:addUserInfo.URL1 forKey:@"url1"];
	}
	if(addUserInfo.NOTE != nil && addUserInfo.NOTE != NULL)
	{
		[bodyObject setObject:addUserInfo.NOTE forKey:@"note"];
	}
	...
	if ([rtcode isEqualToString:@"0"]) {	// success
		...
		if ([rtType isEqualToString:@"map"]) {
			...
			//	userData.CHANGEDATE = lastSyncDate;
				...
				[userData setValue:lastSyncDate forKey:@"OEMCREATEDATE"];
				[userData setValue:lastSyncDate forKey:@"OEMMODIFYDATE"];
				
				DebugLog(@"OEMMODIFYDATE %@", lastSyncDate);
...
-(void)addedContactPhontUpload:(UserInfo*)userData {
	DebugLog(@"addedContactPhontUpload");
...
//		assert(imagePath != nil);
		...
		if (![[NSFileManager defaultManager] fileExistsAtPath:imagePath isDirectory:NO]) {
			// not exist file
			DebugLog(@"imagePath not exist file");
			// TODO: blockview 제거
			[addUserBlockView dismissAlertView];
			[addUserBlockView release];
			return;
...
//		assert(data != nil);
		...
		DebugLog(@"프로필 사진 추가 오류 - 이미지파일 오류. 잘못된 이미지 파일입니다.");
		// TODO: blockview 제거
		[addUserBlockView dismissAlertView];
		[addUserBlockView release];
...
//		self.saveUserData.USERIMAGE =  UIImageJPEGRepresentation(self.addUserImage, 1.0f);
		DebugLog(@"imageUrl %@",[ImageData valueForKey:@"THUMBNAILURL"]);
