...
//	assert(loadimgImageView != nil);
	...
	syncView = [[UIView alloc]initWithFrame:CGRectMake(53, 321, 200, 82)];
	...
	syncTitleLabel2.text = @"주소록 데이터가 많으면 최초 동기화시";
	...
	syncTitleLabel3.text = @"시간이 오래 걸릴 수 있습니다.";
...
#pragma mark ---> 서버 그룹 목록 조회 (getGroups)
-(void)requestGetGroups
{
	
	NSString *initCode = @"Y";
	if([[NSUserDefaults standardUserDefaults] objectForKey:@"joinWebUser"])
	{
...
//	DebugLog(@"\n  ===> initGroup count : %d, RECORDID : %@", [initGroup count], [initGroup objectAtIndex:0]);
//	NSArray* compareGroup = [GroupInfo findAll];
//	DebugLog(@"\n  ===> compareGroup count : %d, RECORDID : %@", [compareGroup count], [compareGroup objectAtIndex:0]);
	...
	for(GroupInfo* locGroup in initGroup){
		...
		for(GroupInfo* compGroup in compareGroup){
			...
			if([locGroup.GROUPTITLE isEqualToString:compGroup.GROUPTITLE]){
				DebugLog(@"\n  ===> locGroup Title = %@, %d = %d", locGroup.GROUPTITLE, [locGroup.ID intValue], [compGroup.ID intValue]);
				//로컬 그룹 아이디가 서버 그룹 아이디보다 클경우 즉 뭔가 추가가 되었을 경우??
				if([locGroup.ID intValue] > [compGroup.ID intValue]){
					DebugLog(@"TTTTT local Group save ? TTTTT");
					...
					DebugLog(@"TTTTT comp Group save ? TTTTT");
...
//	[varpool release];
	...
	NSLog(@"  ===> lastSyncDateTime = %@", lastSyncDateTime);

	NSLog(@"\n========== (동기화 1단계) OEM Address READ ==========>");
	NSTimeInterval start = 0.0f;
	NSTimeInterval end = 0.0f;
	NSTimeInterval timeGap;

	DebugLog(@"=== read OEM Address START (lastSynctime = %@)", lastSyncDateTime);
	start = [JYUtil timestamp];
	//만약 기존에 가입한 사용자 이라면..
	if(lastSyncDateTime != nil && [lastSyncDateTime length] > 0 && lastSyncDateTime != NULL)
	{
		//새로 생성한 데이터들만 가져오는거
		NSLog(@"새로 생성한 데이터들만 가져온다 %@", lastSyncDateTime);
		contacts = [addrData readOEMContactsAlterData];
	}else{
		NSLog(@"=========> readFullOEMContacts   <=============");
		contacts = [addrData readFullOEMContacts];
	//	contacts = [addrData readFullOEMData];
		
	//	NSLog(@"contacts = %i", sizeof(contacts));
	...
	end = [JYUtil timestamp];
	NSLog(@"=== read OEM Address END =====>");
	timeGap = end - start;
	NSLog(@"전체 동기화 목록 읽기 시간 : %f", timeGap);
	...
	for(GroupInfo* oemGroup in [contacts objectForKey:@"GROUP"]){//oem 그룹
		for(GroupInfo* dbGroup in localGroup){ //sqllite DB
			//usay group title == dbgroup title
	...
	lastSyncDateTime = [[NSUserDefaults standardUserDefaults] objectForKey:@"lastSyncDate"];
	NSLog(@"dateTIme = %@", lastSyncDateTime);
	...
	DebugLog(@"LoadingViewController addListContact 호출");
	lastSyncDateTime = [[NSUserDefaults standardUserDefaults] objectForKey:@"lastSyncDate"];
	NSLog(@"dateTIme2 = %@", lastSyncDateTime);
...
-(void)progressControl:(NSNumber*)currentPersent
{
	if(syncState == 1){
		...
		syncTitle2.text = @"주소록 데이터가 많으면 최초 동기화시";

		UILabel* syncTitle3 = (UILabel*)[syncView viewWithTag:kSyncTitleLabel3];
		syncTitle3.text = @"시간이 오래 걸릴 수 있습니다.";
		// ~sochae
...
{
	if([alertView.title isEqualToString:@"동기화 실패"])
		exit(1);
}
...
{
	NSLog(@"\n----- [HTTP] 싱크 주소록 수신 ( addListContactIncludeGroup loadingView ) ----->");
	...
	if (data == nil) {
		...
		[alertView release];
		/*
		[self appDelegate].connectionType = -1;
...
//	NSString *resultData = data.responseData;
	...
	if ([rtcode isEqualToString:@"0"]) {	// success
		...
		DebugLog(@"  ===> [response] RP : %@ ", RP);
		DebugLog(@"  ===> [response] Count : %@", count);
				...
				for(NSString* key in keyArray) {
					if([addressDic objectForKey:@"groupTitle"]){//그룹 정보
						...
						for(int i = 0; i < numIvars; i++) {
							...
							{
							//	DebugLog(@"name = %@", [addressDic objectForKey:key]);
							}
...
//				NSLog(@"%i",nCount);
			...
			/*
			UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"동기화 실패" message:@"어플을 재시작 합니다.(RTTYPE 오류)" delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
...
{
//	NSLog(@"\n========== (동기화 2단계) convert Address Data %@ ==========>", dicData);
	//사용자 정보 저장
...
//	NSString	*body = nil;
	...
	for(GroupInfo *groupData in oemGroupArr)		// oem Address Group
	{
		NSMutableDictionary *group = [[NSMutableDictionary alloc] initWithCapacity:0];
		for(GroupInfo* localgroup in compareGroup)	// local DB Group
		{
	...
	for(UserInfo *userData in [dicData objectForKey:@"PERSON"]){
		...
		for(int i = 0; i < numIvars; i++) {
			...
			if([userData valueForKey:key] != nil){
				...
				if([key isEqualToString:@"OEMCREATEDATE"])
				{
					...
					NSString *newDate = [userData valueForKey:key];
		...
		DebugLog(@"최초 동기화 추가 개수 = %d", [contactList count]);
		[contact release];
	}
...
//	float currentPosition = 0.5;
	syncState = 1;
	NSLog(@"=====> progress pos chaned : %f", currentPosition);
	[self performSelectorInBackground:@selector(progressControl:) withObject:[NSNumber numberWithFloat:currentPosition]];
	// 노티피케이션처리관련 등록..
...
//	NSString* revisionPoint = [[NSUserDefaults standardUserDefaults] objectForKey:@"RevisionPoints"];
	//rp 로그 이외에는 쓰이지가 않아서 주석 처리함.. 2011.03.08
//	if(revisionPoint == nil) revisionPoint = @"0";
	...
	//웹에서 가입한 유저들은 RP=0 initcode=N alterOem 으로 진행
...
*/
	...
	if([[NSUserDefaults standardUserDefaults] objectForKey:@"joinWebUser"])
	{
		...
		NSLog(@"=======웹에서 가입한 유저가 아니다.....============");
		initializeCode =[NSString stringWithFormat:@"Y"];
	}
	*/
	//웹에서 가입한 유저는 건수가 0건으로 동기화 시킨다..
	...
	DebugLog(@"contactDic = %@", contactDic);
...
//	NSLog(@"listBody = %@", listbody);
	...
	DebugLog(@"\n========== (동기화 3단계) OEM Address 전송 ==========>");
//	DebugLog(@"\n  ===> [plist] RP : %@ ", revisionPoint);
	DebugLog(@"\n  ===> [plist] Count : %d", [contactList count]);
	NSLog(@"\n  ===> [plist] lastsynctime : %@ (%@) ", [JYUtil loadFromUserDefaults:kLastSyncTime], [[NSUserDefaults standardUserDefaults] objectForKey:@"lastSyncDate"]);
...
//	NSLog(@"adlistContact body = %@", bodyObject);
	
	NSLog(@"[sochae] LoadingViewController ( cnt = %d )", [contactList count]);
	
	
	USayHttpData *data = [[[USayHttpData alloc]initWithRequestData:@"addListContactIncludeGroup" andWithDictionary:bodyObject timeout:180]autorelease];
...
#pragma mark < ----- [DB] Address Data local DB 저장 (RP 및 lastsynctime 세팅) ----- >
...
//	[self performSelectorInBackground:@selector(progressControl:) withObject:[NSNumber numberWithFloat:0.0]];
	DebugLog(@"\n  ===> sync RP : %@", revisionPoint);
	DebugLog(@"\n  ===> sync groupData count : %d", [groupData count]);
	DebugLog(@"\n  ===> sync userData count : %d", [userData count]);
...
//					NSLog(@"DB insert UserInfo");
				...
				//부분 동기화
					...
					NSLog(@"부분동기화 = %@", [[NSUserDefaults standardUserDefaults] objectForKey:@"RevisionPoints"]);
					[trans commit];
...
//	NSLog(@"GroupInfo DB insert groupcount = %i", [groupData count]);
	...
	if([trans hasError] == NO){
		...
		DebugLog(@"\n==========> End Database Sync and rp setting <================");
		[self performSelectorInBackground:@selector(progressControl:) withObject:[NSNumber numberWithFloat:1.0]];
