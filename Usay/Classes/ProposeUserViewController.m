...
-(void)showImage:(NSString*)cellImage smallImg:(UIImage*)img
{
	...
	[imageView release];   // sochae 2010.12.29 - cLang
	//release 2010.12.28
	[tmpImgView release];
...
//리로드 노티 
-(void)reloadPropose:(NSNotification*)noti
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:@"reloadPropose" object:nil];	//kjh 2011.02.08
...
- (void)viewDidLoad {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reloadPropose:) 
												 name:@"reloadPropose" object:nil];
...
-(void)addAction:(id)sender
{
	...
	else if(LOGINSUCESS != YES)
	{
		...
		if(LOGINEND != YES && LOGINSTART == YES)
		{
	...
	if([[[NSUserDefaults standardUserDefaults] objectForKey:@"PROPOSE"] isEqualToString:@"ISSEND"])
	{
		UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"추천 친구 에러" message:@"잠시 후에 다시 이용 해 주세요" delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
		[alertView show];
		[alertView release];
		return;
					...
					if ([parentParentView isKindOfClass:[CustomCell class]]) {
						...
						if (cell) {
							if (cell.pKey && [cell.pKey length] > 0) {
								...
								[[NSUserDefaults standardUserDefaults] setObject:@"ISSEND" forKey:@"PROPOSE"];
								
							
								//2011.02.07 성공했을 경우에만 테이블에서 제외해야 하는게 필요할듯 하다...
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (nodeCount > 0) {
		...
		DebugLog(@"userData.nickName = %@ pkey = %@", userData.nickName, userData.pkey);
