...
-(void)showImage:(NSString*)cellImage
{
	...
	{
		return;
		imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 480, 300)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([[self appDelegate].msgArray count] < indexPath.row) {
		...
		if ([msgData.msgType isEqualToString:@"T"]) {			// 텍스트
		...
		} else if ([msgData.msgType isEqualToString:@"P"]) {	// 사진
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if(editingStyle == UITableViewCellEditingStyleDelete) {
		...
		@synchronized([self appDelegate].msgArray) {
			...
			[[[self appDelegate] msgArray] removeObjectAtIndex:indexPath.row];
				...
				if (buttonIndex == 0) {	// 재전송
					...
					@synchronized([self appDelegate].msgArray) {
						...
						if ([msgData.msgType isEqualToString:@"T"]) {
							...
							[[self appDelegate].msgArray removeObjectAtIndex:reSendCellRow];
							////////////////////////////////////////////////////////////////////////////////////////
							...
							if (noticeDateMsg) {
								...
								[MessageInfo findWithSqlWithParameters:
								 ...
								 self.sessionKey, @"", noticeDateMsg, @"D", currentTimeNumber, @"", @"", @"", @"", @"1", [NSNumber numberWithInt:0], nil];
						...
						//	NSLog(@"count = %d", [paths count]);
							...
							if ([HttpAgent mc] == nil || [HttpAgent cs] == nil) {
								...
								transParam = [[NSString alloc ] initWithFormat:@"https://social.paran.com/KTH/createNewMessage.json?pKey=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=P&sessionKey=%@&uuid=%@", 
#else
								transParam = [[NSString alloc ] initWithFormat:@"https://dev.usay.net/KTH/createNewMessage.json?pKey=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=P&sessionKey=%@&uuid=%@", 
#endif // #ifdef DEVEL_MODE
											  pkey, 
											  [HttpAgent ucmc], [HttpAgent uccs], [[self appDelegate] getSvcIdx], self.sessionKey, uuid];
							} else {
#ifdef DEVEL_MODE	// sochae 2010.09.09 - dev server
								transParam = [[NSString alloc ] initWithFormat:@"https://social.paran.com/KTH/createNewMessage.json?pKey=%@&MC=%@&CS=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=P&sessionKey=%@&uuid=%@", 
#else
								transParam = [[NSString alloc ] initWithFormat:@"https://dev.usay.net/KTH/createNewMessage.json?pKey=%@&MC=%@&CS=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=P&sessionKey=%@&uuid=%@", 
#endif // #ifdef DEVEL_MODE
							...
							if ([HttpAgent mc] == nil || [HttpAgent cs] == nil) {
#ifdef DEVEL_MODE	// sochae 2010.09.09 - dev server
								transParam = [[NSString alloc ] initWithFormat:@"https://social.paran.com/KTH/createNewMessage.json?pKey=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=V&sessionKey=%@&uuid=%@", 
#else
								transParam = [[NSString alloc ] initWithFormat:@"https://dev.usay.net/KTH/createNewMessage.json?pKey=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=V&sessionKey=%@&uuid=%@", 
#endif // #ifdef DEVEL_MODE
											  pkey, 
											  [HttpAgent ucmc], [HttpAgent uccs], [[self appDelegate] getSvcIdx], self.sessionKey, uuid];
							} else {
#ifdef DEVEL_MODE	// sochae 2010.09.09 - dev server
								transParam = [[NSString alloc ] initWithFormat:@"https://social.paran.com/KTH/createNewMessage.json?pKey=%@&MC=%@&CS=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=V&sessionKey=%@&uuid=%@", 
#else
								transParam = [[NSString alloc ] initWithFormat:@"https://dev.usay.net/KTH/createNewMessage.json?pKey=%@&MC=%@&CS=%@&UCMC=%@&UCCS=%@&svcidx=%@&mType=V&sessionKey=%@&uuid=%@", 
#endif // #ifdef DEVEL_MODE
											  pkey, [HttpAgent mc], [HttpAgent cs],
											  [HttpAgent ucmc], [HttpAgent uccs], [[self appDelegate] getSvcIdx], self.sessionKey, uuid];
							}
	...
	//		NSLog(@"=======Image Size %d========", [image ]);
			...
			if (noticeDateMsg) {
				...
				[MessageInfo findWithSqlWithParameters:
				 ...
				 "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
				 self.sessionKey, [[self appDelegate] generateUUIDString], noticeDateMsg, @"D", currentTimeNumber, @"", @"", @"", @"", @"1", [NSNumber numberWithInt:0], [imagePath lastPathComponent], nil];
		...
		if (videoURL) {
			...
			if (noticeDateMsg) {
				...
				[MessageInfo findWithSqlWithParameters:
				 ...
				 "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
				 self.sessionKey, [[self appDelegate] generateUUIDString], noticeDateMsg, @"D", currentTimeNumber, @"", @"", @"", @"", @"1", [NSNumber numberWithInt:0], [backupMoviePath lastPathComponent], nil];
...
- (IBAction) clicked:(id)sender
{
	...
	else if (sender == sendBtn)	// 대화 [보내기] 버튼
	{
		...
		if (noticeDateMsg) {
			...
			[MessageInfo findWithSqlWithParameters:
			 @"insert into _TMessageInfo(CHATSESSION, MESSAGEKEY, MSG, MSGTYPE, REGDATE, PKEY, NICKNAME, PHOTOURL, ISSENDMSG, ISSUCCESS, READMARKCOUNT) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
			 self.sessionKey, [[self appDelegate] generateUUIDString], noticeDateMsg, @"D", currentTimeNumber, @"", @"", @"", @"", @"1", [NSNumber numberWithInt:0], nil];
...
- (void)inviteButtonClicked
{
	...
	if (msgListData) {
		...
		if (buddyUserInfoArray && [buddyUserInfoArray count] > 0) {
	...
	//	CGRect rect = self.view.frame;
		...
		if(parentView.window.frame.size.width == 320)
		 rect = CGRectMake(0, 0, parentView.window.frame.size.width, parentView.window.frame.size.height-self.navigationController.navigationBar.frame.size.height-20);
			...
			rect = CGRectMake(0, 0, parentView.window.frame.size.width, parentView.window.frame.size.height-self.navigationController.navigationBar.frame.size.height-40);
		...
		if (chatSession && [chatSession length] > 0) {
			...
			if ([newBuddyArray count] > 0) {
				if ([newBuddyArray count] > 1) {
					...
					UIFont *titleFont = [UIFont systemFontOfSize:20];
					CGSize titleStringSize = [[NSString stringWithFormat:@"그룹채팅 (%i명)", [newBuddyArray count]+1] sizeWithFont:titleFont constrainedToSize:(CGSizeMake(rect.size.width-120.0, MAXFLOAT))];
					UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, titleStringSize.width, 20.0f)];
					[titleLabel setFont:titleFont];
					titleLabel.textAlignment = UITextAlignmentCenter;
					[titleLabel setTextColor:ColorFromRGB(0x053844)];
					[titleLabel setBackgroundColor:[UIColor clearColor]];
					[titleLabel setText:[NSString stringWithFormat:@"그룹채팅 (%i명)", [newBuddyArray count]+1]];
					[titleView addSubview:titleLabel];	
					[titleLabel release];
					self.navigationItem.titleView = titleView;
					[titleView release];
			...
			for (CellMsgListData * msgListData in [self appDelegate].msgListArray) {
				if ([msgListData.chatsession isEqualToString:sessionKey]) {
					if (msgListData.userDataDic && [msgListData.userDataDic count] > 0) {
						UIView *titleView = [[UIView alloc] initWithFrame:self.navigationItem.titleView.bounds];
						UIFont *titleFont = [UIFont systemFontOfSize:20];
						CGSize titleStringSize = [[NSString stringWithFormat:@"그룹채팅 (%i명)", [msgListData.userDataDic count] + [inviteBuddyArray count] + 1] sizeWithFont:titleFont constrainedToSize:(CGSizeMake(rect.size.width-120.0, MAXFLOAT))];
						UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, titleStringSize.width, 20.0f)];
						[titleLabel setFont:titleFont];
						titleLabel.textAlignment = UITextAlignmentCenter;
						[titleLabel setTextColor:ColorFromRGB(0x053844)];
						[titleLabel setBackgroundColor:[UIColor clearColor]];
						[titleLabel setText:[NSString stringWithFormat:@"그룹채팅 (%i명)", [msgListData.userDataDic count] + [inviteBuddyArray count] + 1]];
						[titleView addSubview:titleLabel];	
						[titleLabel release];
						self.navigationItem.titleView = titleView;
						[titleView release];
...
//		sayTableView = [[[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-INPUTVIEW_CY)] autorelease];
	...
	if (inputTextView == nil) {
		...
		keyboardUpTimer = [NSTimer scheduledTimerWithTimeInterval:0.6 target:self 
...
- (void)willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    {  
        ...
        case UIDeviceOrientationPortrait: 
		{
			...
			CGRect inputViewFrame = inputView.frame;
			CGRect sayTableViewFrame = sayTableView.frame;
			sayTableViewFrame.size.width = 320;
			if (keyboardShown) {
				sayTableViewFrame = CGRectMake(0, 0, 320, 161);
				//sayTableViewFrame.size.height = 320 - inputViewFrame.size.height - PORTRAIT_KEYBOARD_HEIGHT;
			} else {
				sayTableViewFrame = CGRectMake(0, 0, 320, 377);
			//	sayTableViewFrame.size.height = 416 - INPUTVIEW_CY;
			}
			
			sayTableView.frame = sayTableViewFrame;
			
			if (keyboardShown) {
				inputViewFrame = CGRectMake(0, 161, 320, 39);
				//	inputViewFrame.origin.y = 416 - PORTRAIT_KEYBOARD_HEIGHT - inputViewFrame.size.height;
			} else {
				inputViewFrame = CGRectMake(0, 377, 320, 39);
			//	inputViewFrame = CGRectMake(0, rect.size.height-INPUTVIEW_CY, rect.size.width, INPUTVIEW_CY)];	// sochae 2010.09.11 - INPUTVIEW_CY
			//	inputViewFrame.origin.y = 416 - inputViewFrame.size.height;
			}
		//	inputViewFrame.size.width = 300;
			inputView.frame = inputViewFrame;
		...
		//	CGRect inputTextViewFrame = inputTextView.frame;
		//	inputTextViewFrame.size.width =  150;//rect.size.width-110;
			...
			CGRect sendBtnFrame = sendBtn.frame;
			sendBtnFrame.origin.x = 320-58; //rect.size.width-58;
			sendBtnFrame.origin.y = inputView.frame.size.height - 33;
			sendBtn.frame = sendBtnFrame;
			 */
		}
...
/*				
				inputView.frame= CGRectMake(0, 15, 480, 39);
				inputTextView.frame=CGRectMake(50, 5, 210, 29);
				sayTableView.frame=CGRectMake(0, 0, 480, 15);
*/
 }
			...
			CGRect inputViewFrame = inputView.frame;
			CGRect sayTableViewFrame = sayTableView.frame;
			sayTableViewFrame.size.width = rect.size.width; //480
			...
			if (keyboardShown) {
				NSLog(@"keyboard show");
				inputViewFrame.origin.y =  15;//rect.size.height - LANDSCAPE_KEYBOARD_HEIGHT - inputViewFrame.size.height;  //416-162
				
			} else {
				inputViewFrame.origin.y =  320 - INPUTVIEW_CY-self.navigationController.navigationBar.frame.size.height;
			}
			NSLog(@"inputViewFrame = %f", inputViewFrame.origin.y);
			
			inputViewFrame.size.width =  480;//rect.size.height;
			inputView.frame = inputViewFrame;
			
			
			if (keyboardShown) {
				sayTableViewFrame.size.height = 15;  //rect.size.width - inputView.frame.size.height - LANDSCAPE_KEYBOARD_HEIGHT;
			} else {
				sayTableViewFrame.size.height = 320 - INPUTVIEW_CY-self.navigationController.navigationBar.frame.size.height;
			}
			sayTableViewFrame.size.width= 480;
			
			sayTableView.frame = sayTableViewFrame;
			
			
			NSLog(@"sayVir = %f", sayTableView.frame.size.height);
        ...
        case UIDeviceOrientationLandscapeRight:  
		{
			...
			sayTableViewFrame.size.width = rect.size.width;
			if (keyboardShown) {
				inputViewFrame.origin.y = rect.size.height - LANDSCAPE_KEYBOARD_HEIGHT - inputViewFrame.size.height;
			} else {
				inputViewFrame.origin.y = rect.size.height - inputViewFrame.size.height;
			}
			sayTableView.frame = sayTableViewFrame;
        ...
        case UIDeviceOrientationPortraitUpsideDown:  
		{
			...
			sayTableViewFrame.size.width = rect.size.width;
			if (keyboardShown) {
				sayTableViewFrame.size.height = rect.size.height - inputViewFrame.size.height - PORTRAIT_KEYBOARD_HEIGHT;
			} else {
				sayTableViewFrame.size.height = rect.size.height - inputViewFrame.size.height;
			}
			sayTableView.frame = sayTableViewFrame;
			...
			if (keyboardShown) {
				inputViewFrame.origin.y = rect.size.height - PORTRAIT_KEYBOARD_HEIGHT - inputViewFrame.size.height;
			} else {
				inputViewFrame.origin.y = rect.size.height - inputViewFrame.size.height;
			}
			inputViewFrame.size.width = rect.size.width;
		...
		case UIDeviceOrientationFaceUp:
		{
			...
			CGRect sayTableViewFrame = sayTableView.frame;
			sayTableViewFrame.size.width = rect.size.width;
			sayTableViewFrame.size.height = rect.size.height - inputView.frame.size.height - keyboardHeight;
			sayTableView.frame = sayTableViewFrame;
			
			inputViewFrame.origin.y = rect.size.height - keyboardHeight;
			inputViewFrame.size.width = rect.size.width;
			inputView.frame = inputViewFrame;
