...
//		oldRow = [oldIndexPath row];
	...
	//	NSString *newValue = selectedCell.textLabel.text;
		...
		NSInteger first = [newValue rangeOfString:@"("].location;
		NSInteger last;
		NSRange middleRange;
		
		
		//문자열에서 (를 검색해서 그이후 문자열은 삭제시킨다.
		while (first != NSNotFound) {
			NSLog(@"leng = %i", [newValue length] - first);
			NSRange tmpRange = {first+1, [newValue length]-first-1}; //( 이후의 위치값과 그 이후 길이를 구하기 위한 셋팅
			NSLog(@"tmpRange = %i", tmpRange.length);
			...
			last = first;
			NSLog(@"last = %i", last);
			middleRange = [newValue rangeOfString:@"(" options:NSCaseInsensitiveSearch range:tmpRange ];
			
	
			first = middleRange.location;
			
		}
		NSRange endRange = {0, last};
		
		NSString *tmpValue = [[NSString alloc] initWithString:[newValue substringWithRange:endRange]];
		
		NSLog(@"new tmpValue =%@", tmpValue);
		
		
		
		*/
		
		//여기에서 ( ) 위치를 찾아서 제외시켜줘야 한다..
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSLog(@"namesection %d", [nameSection count]);
