...
//	mainPhotoView.layer.borderWidth = 1.0;
	...
	NSLog(@"person info = %@", personInfo.THUMBNAILURL);
...
//		}
	...
	//	NSLog(@"Disable  a=%@ b=%@ c=%@", personInfo.MOBILEPHONENUMBER, personInfo.HOMEPHONENUMBER, personInfo.ORGPHONENUMBER);
		...
		if(personInfo.MOBILEPHONENUMBER == NULL && personInfo.HOMEPHONENUMBER == NULL && personInfo.ORGPHONENUMBER == NULL)
		{
			[smsbutton setEnabled:NO];
		}
		// sochae 2010.10.01
		//if(personInfo.RPKEY == nil && [personInfo.RPKEY length] <= 0)
		if(!(personInfo.ISFRIEND && [personInfo.ISFRIEND isEqualToString:@"S"]))
		// ~sochae
		{
			[sayButton setEnabled:NO];
		}
		
		if([personInfo.MOBILEPHONENUMBER length]<=0 && [personInfo.HOMEPHONENUMBER length]<=0 && [personInfo.ORGPHONENUMBER length]<=0)
		{
			[smsbutton setEnabled:NO];
		}
...
//					assert(titleLabel != nil);
				...
				if(sayViewController != nil) {
					...
					if (personInfo.NICKNAME && [personInfo.NICKNAME length] > 0) { //for
...
-(void)updateDataValue:(NSString*)value forkey:(NSString*)key{
	...
	if([key isEqualToString:@"nickname"]){ //format
		cell.textLabel.textColor = [UIColor blackColor];
		cell.textLabel.text = value;
		personInfo.NICKNAME = value; //format
	}else{
...
- (UITableViewCell *)tableView:(UITableView *)theTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if([rowKey isEqualToString:@"nickname"]){ //
		if(!isEdit){
	...
	if([rowKey isEqualToString:@"nickname"]){ //formated
		...
		if (cell == nil) {
			...
			if(isEdit){
				...
				NSLog(@"photo y =%f", mainPhotoView.frame.origin.y);
				NSLog(@"여기로 들어오면 메인이미지");
...
//		[cell.contentView insertSubview:photoImageView atIndex:0];
    ...
    if([rowKey isEqualToString:@"nickname"]) { //for
		cell.textLabel.text = self.personInfo.NICKNAME; //for
		
		
		
		if([cell.textLabel.text length] <=0)
		{
			cell.textLabel.text = self.personInfo.PROFILENICKNAME;
		}
...
//			SDWebImageManager *manager = [SDWebImageManager sharedManager];	
		...
		if([rowKey isEqualToString:@"MOBILEPHONENUMBER"]){
			...
			NSLog(@"직장전화 = %@", [CheckPhoneNumber convertPhoneInsertHyphen:(NSMutableString*)self.personInfo.ORGPHONENUMBER]);
		...
		NSLog(@"titleLabel = %@", detailTitleLabel.text);
...
-(BOOL)requestUpdateContact{
	//사용자 정보 저장
/*
	if((!personInfo.FORMATTED || [personInfo.FORMATTED length] <= 0) || 
	   ((!personInfo.MOBILEPHONENUMBER || [personInfo.MOBILEPHONENUMBER length] <= 0) && 
	   (!personInfo.IPHONENUMBER || [personInfo.IPHONENUMBER length] <= 0))){
		UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"입력 정보 확인" 
															message:@"이름과 전화번호는 입력값이 있어야 수정 가능합니다." 
...
*/
	if (data == nil) {
		...
		// 200 이외의 에러
		UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"네트워크 접속 에러" 
															message:@"Wi-Fi 또는 3G 연결상태가 좋지 않습니다. 연결 상태를 확인해 주세요.(e0020)" delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
		[alertView show];
		[alertView release];
		return;
	}
