...
-(void)completeProfilePhoto:(NSNotification *)notification
{
	...
	if ([notification object] == nil) {
		...
		if (imagePath && [imagePath length] > 0) {
			if (![[NSFileManager defaultManager] fileExistsAtPath:imagePath isDirectory:NO]) {
				...
				if (!successed) {
					NSLog(@"error profile photo removeItemAtPath %@", [error localizedDescription]);
				}
			}
		}
		...
		if (representPhoto && [self.representPhoto length] > 0) {
			...
			NSLog(@"completeProfilePhoto FAIL representPhoto = %@", representPhoto);
			// Use the cached image immediatly
			UIButton *mainPhotoBtn = (UIButton *)[self.view viewWithTag:ID_MAINPHOTO];
			if(mainPhotoBtn) {
	...
	NSLog(@"completeProfilePhoto OK representPhoto = %@", representPhoto);
}
...
//	NSLog(@"loadProfile response");
	...
	if (data == nil) {
		// 200 이외의 에러
		UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"네트워크 접속 에러" message:@"Wi-Fi 또는 3G 연결상태가 좋지 않습니다. 연결 상태를 확인해 주세요.(e0034)" delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
		[alertView show];
		[alertView release];
		return;
	}
		...
		if ([rtType isEqualToString:@"map"]) {
			...
			if (representPhotoUrl && [representPhotoUrl length] > 0) {
				if (representPhoto) {
					...
					NSLog(@"loadProfile representPhoto = %@", representPhoto);
				} else {
					representPhoto = [[NSMutableString alloc] initWithString:representPhotoUrl];
					NSLog(@"loadProfile representPhoto = %@", representPhoto);
				}
