...
{	
	...
	if ([searchString length] > 0) {
		...
		for(CellMsgListData *msgListData in [[self appDelegate] msgListArray]) {
			...
			if ([nickName length] < [searchString length]) {
				// skip
			} else {
				if (nSearchType == 0) {
					...
					NSRange range = [nickName rangeOfString:searchString];
					if (range.location != NSNotFound) {
						[self.msgSearchArray addObject:msgListData];
					} else {
						// skip
...
-(void)sayButtonClicked
{
	if ([self appDelegate].connectionType == -1) {	// -1:offline mode 0:disconnect 1:Wi-Fi 2:3G
		...
		UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"네트워크 접속 에러" message:@"3G 또는 Wi-Fi 연결상태가 좋지 않습니다. 연결 상태를 확인해 주세요.(e0026)" delegate:nil cancelButtonTitle:@"확인" otherButtonTitles:nil, nil];
		[alertView show];
		[alertView release];
		return;
	}
