...
-(void)reloadAddressData {
	NSLog(@"reload EditAddressViewcontroller");
	...
	NSLog(@"All User Cnt %d", [allNames count]);
...
{
	numberofEdit = index;
	DebugLog(@"editumber = %d", numberofEdit);
...
//	assert(titleView != nil);
	...
	DebugLog(@"didload edit number = %d", numberofEdit);
	if(numberofEdit	 == 0)
	{
...
//	titleLabel.shadowColor = [UIColor whiteColor];
	...
	*/
	...
	UIImage* leftImage = [UIImage imageNamed:@"btn_addressbook_del.png"];
	UIImage* leftfocusImage = [UIImage imageNamed:@"btn_addressbook_del.png"];
	...
	*/
	...
	UIImage* rightImage = [UIImage imageNamed:@"btn_addressbook_move.png"];
	UIImage* rightfocusImage = [UIImage imageNamed:@"btn_addressbook_move.png"];
...
-(void)editData:(NSInteger)sender
{
		...
		self.buttonToolBar.hidden = NO;
			...
			if(self.customGroupTableView == nil){
				...
				DebugLog(@"view height y= %f %f", viewBounds.size.height, viewBounds.origin.y);
...
-(void)editGroupClicked2:(NSInteger)sender {
	...
	DebugLog(@"number of edit =%d", numberofEdit);
...
//		key = [keyArray objectAtIndex:section];
		...
		if (dataItem.ISFRIEND && [dataItem.ISFRIEND isEqualToString:@"S"] || dataItem.ISFRIEND && [dataItem.ISFRIEND isEqualToString:@"A"]) {
		...
		if([dataItem.ISFRIEND isEqualToString:@"S"] || [dataItem.ISFRIEND isEqualToString:@"A"]){// 친구일경우 접속 상태 정보 표시 // 0:offline  1:online  2:busy (바빠요)  8:곧 돌아옵니다.  16:다른용무중
		...
		if(dataItem.CHANGEDATE != nil && [dataItem.CHANGEDATE length] > 0){
			if([[dataItem.CHANGEDATE substringToIndex:8] isEqualToString:dateOftoday]){//변경된 일자가 오늘일경우
				if([[dataItem.ISFRIEND uppercaseString] isEqualToString:@"S"] || [dataItem.ISFRIEND isEqualToString:@"A"]){//서로 친구일경우
...
//#if TARGET_IPHONE_SIMULATOR		
		DebugLog(@"select indexno %i",[indexPath row]);
		DebugLog(@"select section %i",[indexPath section]);
//#endif
...
//		key = [[self.peopleInGroup allKeys] objectAtIndex:section];
		...
		for(UserInfo* OnOffuser in nameSection){
			if([OnOffuser.ISFRIEND isEqualToString:@"S"] || [OnOffuser.ISFRIEND isEqualToString:@"A"]){
...
-(void)requestDeleteGroup:(NSString*)include withGroupId:(NSString*)gid{
	...
	DebugLog(@"body = %@", bodyObject);
	DebugLog(@"deleteGroup >> %@",[json stringWithObject:bodyObject error:nil]);
	[json release];
...
-(void)deleteGroup:(NSNotification *)notification {
	...
	//죽는다 예외처리 하자
	if(editGroupBlock !=nil && editGroupBlock != NULL)
	{
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	DebugLog(@"editGroup = %@", [self.allGroups objectAtIndex:btnIndex]);
	
	DebugLog(@"action title = %@", selectedButtonTitle);
